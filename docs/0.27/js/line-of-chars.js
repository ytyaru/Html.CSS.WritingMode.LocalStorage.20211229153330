...
function initLineOfChars() { // UI操作でフォントサイズを変更する
    ...
    function setLineOfCharsToHtml(value) { // CSSの変数をHTMLにセットする
        ...
        setFontSizePixel(parseInt(document.querySelector('#columns').value), document.querySelector('#writing-mode').value, parseInt(value), parseFloat(document.querySelector('#letter-spacing').value));
//        setFontSizePixel(document.querySelector('#columns').value, document.querySelector('#writing-mode').value, value, document.querySelector('#letter-spacing').value);
