...
//        p.style.setProperty('break-after', 'always');
        ...
        // 見切れる位置の計算がうまくできない・・・・・・・・・・
        // 原因はoffsetWidthの取得値が折り返したテキスト文を計算してくれないらしいせい。
        // https://developer.mozilla.org/ja/docs/Web/API/HTMLElement/offsetLeft
        console.log(p.getClientRects());

        let beforeP = null;
        if (parseInt(getComputedStyle(document.querySelector(':root')).getPropertyValue('--client-height')) <= (START + LINE_OF_PX)) {
//        if (document.body.clientHeight <= (START + LINE_OF_PX)) {
//        if ((document.body.clientHeight - START) <= LINE_OF_PX) {
//        if (START <= LINE_OF_PX) {
