...
function calcFontSizePixel() { // フォントサイズをピクセル単位で算出する
    ...
    //const LINE_OF_PX = (IS_VERTICAL) ? MAIN.clientHeight : MAIN.clientWidth; // １行の表示領域 684ms
    const LINE_OF_PX = Css.Main.get('inline-size'); // １行の表示領域 なぜかこれで高速化した
    //const COL_COUNT= parseInt(Css.Root.get('--column-count'));
