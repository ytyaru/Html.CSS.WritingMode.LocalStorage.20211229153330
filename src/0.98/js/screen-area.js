function defineScreen() { // 画面領域・役割・クリックしたときの処理
    ...
    _Screen.prototype.on = function(x, y) {
        ...
        */
        switch (self.roll(x, y)) {
            case self.Roles.PagingForward: return (Paging.Page < Paging.Count) ? Paging.Page++ : null;
            case self.Roles.PagingBack: return (1 < Paging.Page) ? Paging.Page-- : null;
    ...
    _Screen.prototype.cursor = function(x, y) {
        ...
        switch (ROLE) {
            case self.Roles.PagingForward: return (Paging.Count === Paging.Page) ? 'auto' : getCssCursor(getForwardIconUrl(), getCursorName(ROLE));
            case self.Roles.PagingBack: return (1 === Paging.Page) ? 'auto' : getCssCursor(getBackIconUrl(), getCursorName(ROLE));
