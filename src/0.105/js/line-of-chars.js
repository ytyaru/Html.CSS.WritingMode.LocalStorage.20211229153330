...
class BaseWidget {
    constructor(id) {
        if(document.getElementById(id)) { throw `ID重複エラー。「${id}」は既存です。他のIDを指定してください。`; }
        this._id = id;
    }
    ...
    load() { return localStorage.getItem(this._id) || Css.Root.get(`--${this._id}`); }
    save() { this.saveStorage(); }
    saveStorage() { localStorage.setItem(this._id, this.Element.value); }
}
...
class LineOfChars extends BaseWidget {
    constructor() {
        super('line-of-chars');
        this.load();
    }
    load() {
        this.Element.value = super.load() || 30;
        this.Element.min = Css.Root.get(`--min-${this.Id}`) || 15;
        this.Element.max = this._calcMaxLineOfChars() || Css.Root.get(`--max-${this.Id}`) || 30;
        this.Element.addEventListener('input', e => {
            // 値セット
            Css.Root.set(`--${this.Id}`, `${e.target.value}`);
            Html.id(`${this.Id}-label`).innerHTML = e.target.value;
            console.debug(`${e.target.value}字／行`);
            // フォントサイズ変更処理
            setFontSizePixel();
            // 変更後にページ再計算
            Paging.break();
    ...
    _calcMaxLineOfChars() { // 解像度と画面の向きから最小フォントサイズ（10px）字の最大字数／行を算出する（但しCSSで指定した最大値より大きくはならない）
        ...
        return Math.min(Math.floor(LINE_OF_PX / MIN_FONT_SIZE_PX), Css.Root.get(`--max-${this.Id}`));
    }
