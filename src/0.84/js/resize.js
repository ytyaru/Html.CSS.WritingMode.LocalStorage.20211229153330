...
//    setCss('--line-of-px', Math.floor(cssF((IS_VERTICAL) ? 'height' : 'width', 'main:not([hidden])'))); // 1文字入らないのでちょっと小さくする
    ...
    console.log(`main inline-size:${cssF('inline-size', 'main:not([hidden])')}`)
    console.log(`main height:${cssF('height', 'main:not([hidden])')}`)
    console.log(`main width:${cssF('width', 'main:not([hidden])')}`)
