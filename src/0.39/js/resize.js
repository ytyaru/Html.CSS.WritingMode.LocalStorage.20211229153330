...
function calcScreenSize(writingMode, columns) { // 画面比率を変える（writingModeと段組数に応じて）
    ...
    */
    ...
    const C_H = document.documentElement.clientHeight;
    const C_W = document.body.clientWidth;
    const B_H = C_H / (IS_VERTICAL) ? columns : 1;
    const B_W = C_W / (IS_VERTICAL) ? 1 : columns;
    root.style.setProperty('--width', `${B_W}px`);
    root.style.setProperty('--height', `${B_H}px`);
    console.log(`C_W:${C_W}, C_H:${C_H}, B_W:${B_W}, B_H:${B_H}`);
}
