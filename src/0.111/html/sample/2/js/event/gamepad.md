...
　左からSELECT, STARTボタンの順である。プレステやスーファミより。
...
　なお、配置がずれたら別の名前にすべき。たとえばメインボタンが①〜⑥になったゲームパッドの場合など。

　また、A,B,X,Yの名前を使いつつも配置が違うゲームパッドもある。物理的な位置と名前がいつも一致していてほしい。そこで東西南北の頭文字を使ってみる。North, South, East, West。
...
　中央のSELECT,STARTも重複しない英字１字にできないか。
...
```
```
ABC  FGHIJK M OPQ  T V XYZ
```

　SELECT,STARTボタンは大抵、ポーズ（一時停止）させたり、メニューを表示したりする。その頭文字を使ってみる。
...
```
ABC  FGHIJK   O Q  T V XYZ
```

　L1,L2,R1,R2も１桁にしたい。残っているキーF〜Kを使う。FをL1にする。つぎのL2はアルファベット順のGを使う。キーボードのホームポジションでも隣である。JをR1にする。つぎのR2はアルファベット順のKを使う。キーボードのホームポジションでも隣である。
...
AXES[0] AXES[L][H] = -1.0 〜 0.0 〜+1.0
AXES[1] AXES[L][V] = -1.0 〜 0.0 〜+1.0
AXES[2] AXES[R][H] = -1.0 〜 0.0 〜+1.0
AXES[3] AXES[R][V] = -1.0 〜 0.0 〜+1.0
...
AXES[0] AXES[L][SIDE] = -1.0 〜 0.0 〜+1.0
AXES[1] AXES[L][TOP]  = -1.0 〜 0.0 〜+1.0
AXES[2] AXES[R][SIDE] = -1.0 〜 0.0 〜+1.0
AXES[3] AXES[R][TOP]  = -1.0 〜 0.0 〜+1.0
