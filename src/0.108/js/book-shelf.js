...
function filterIndexList() {
    ...
    for (const id of ['genre', 'rating', 'tag', 'volume', 'completed'].map(v=>`${v}-filter`)) {
        ...
        if ('volume-filter' === event.target.id) {
            ...
            datas = datas.filter(d=> min <= d['chars'] && d['chars'] <= max)
        }
        else if ('completed-filter' === event.target.id) {
            datas = datas.filter(d=>(1 === value) ? d['completed'] : !d['completed']);
        }
        else if ('tag-filter' === event.target.id) {
            datas = datas.filter(d=>d['tag'].includes(value));
        }
        else if ('rating-filter' === event.target.id) {
            console.log('rating:', value)
            datas = datas.filter(d=>d['rating'][value] < 1);
        }
        else { datas = datas.filter(d=>d[key]===value); }
        */
    }
    return datas;
    //updateIndexList(datas);
