...
//    const COL_GAP_PX = ((IS_VERTICAL) ? window.screen.availHeight : window.screen.availWidth) * (parseFloat(getComputedStyle(document.querySelector(':root')).getPropertyValue('--column-gap')) * 0.01);
...
//    let fontSizePx = (LINE_OF_PX - COL_GAP_PX) / (lineOfChars * COLUMNS); // 入力した字／行と字間からピクセル単位でフォントサイズを算出する
    let fontSizePx = (LINE_OF_PX) / (lineOfChars * COLUMNS); // 入力した字／行と字間からピクセル単位でフォントサイズを算出する
    fontSizePx -= (fontSizePx * letterSpacing);
