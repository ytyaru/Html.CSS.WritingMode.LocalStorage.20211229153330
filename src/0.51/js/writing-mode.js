...
//        const oldHeadElement = document.querySelector(`main:not([hidden]) > p[page="${cssI('--page-index')}"]`);
//        const oldHeadElement = getHeadElement(); // scroll-position-element.js WritingMode変更前に実行する
        const a = getHeadElement(); // scroll-position-element.js WritingMode変更前に実行する
        console.log(a);
        const oldHeadElement = document.querySelector('#head-element');
        ...
        oldHeadElement.scrollIntoView(true); // 変更前の先頭要素へ移動する
//        breakPage();
        // 現在ページ数は？　どうやって計算する？
        const newHeadElement = getHeadElement(); // scroll-position-element.js WritingMode変更前に実行する
        oldHeadElement.scrollIntoView(true);
        const NOW_PAGE = newHeadElement.getAttribute('page'); // p要素が複数カラムにまたがっていたら先頭位置になる
        document.querySelector(':root').style.setProperty('--page-index', NOW_PAGE);
        /*
        */
    });
    setInputRangeWritingMode();
