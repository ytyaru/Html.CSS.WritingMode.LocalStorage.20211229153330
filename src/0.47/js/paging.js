...
function firstPage() { movePageAbsolute(1); calcPage();  }
...
//    const TARGET_PAGE = (0 < page) ? page : cssI('--page-length') + page;
    let TARGET_PAGE = (0 < page) ? page : cssI('--page-length') + page;
    if (page < 1) { TARGET_PAGE = 1; }
    if (page > cssI('--page-length')) { TARGET_PAGE = cssI('--page-length'); }
    let TARGET_PAGE = page; 
    if (page < 0) {
        TARGET_PAGE = cssI('--page-length') + page;
        if (TARGET_PAGE < 1) { TARGET_PAGE = 1; }
    }
    const TARGET_PAGE = (0 < page) ? page : cssI('--page-length') + page;
    const TARGET = document.querySelector(`p[page='${TARGET_PAGE}']`);
    console.log(TARGET_PAGE, TARGET);
    if (TARGET) {
        TARGET.scrollIntoView(true);
        document.querySelector(':root').style.setProperty('--page-index', TARGET_PAGE);
    }
