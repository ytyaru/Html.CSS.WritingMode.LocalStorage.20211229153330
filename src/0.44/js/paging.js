...
//    const PAGE_PX = (cssF(((IS_VERTICAL) ? 'height' : 'width'), 'body') + COL_GAP_PX) * increment * COLUMN_COUNT;
//    const PAGE_PX = (cssF(((IS_VERTICAL) ? 'height' : 'width'), 'body') + COL_GAP_PX + (COL_RULE_WIDTH_PX * (0<increment) ? 1 : -1) + ((COL_RULE_WIDTH_PX/2)*((0<increment) ? -1 : 1 ))) * increment * COLUMN_COUNT;
    const MAIN = document.querySelector('main:not([hidden])');
//    const PAGE_PX = (IS_VERTICAL) ? MAIN.clientHeight : MAIN.clientWidth; // １行の表示領域
//    const PAGE_PX = ((IS_VERTICAL) ? MAIN.clientHeight : MAIN.clientWidth) + COL_GAP_PX; // １行の表示領域
//    const PAGE_PX = (((IS_VERTICAL) ? MAIN.clientHeight : MAIN.clientWidth) + COL_GAP_PX ) * increment; // １行の表示領域
//    const PAGE_PX = ((((IS_VERTICAL) ? MAIN.clientHeight : MAIN.clientWidth)) * increment) + (COL_GAP_PX*(increment)) + (((0<increment) ? 0 : 1) * COLUMN_COUNT); // １行の表示領域

//    const PAGE_PX = ((((IS_VERTICAL) ? MAIN.clientHeight : MAIN.clientWidth) + COL_GAP_PX) * increment) + ((1<COLUMN_COUNT) ? 0.5 : 0) + (((0<increment) ? 0 : 1) * COLUMN_COUNT); // １行の表示領域

//    const PAGE_PX = cssF(((IS_VERTICAL) ? 'height' : 'width'), 'main:not([hidden])') * increment;
//    const PAGE_PX = (cssF(((IS_VERTICAL) ? 'height' : 'width'), 'main:not([hidden])') + COL_GAP_PX) * increment;

//    const PAGE_PX = (cssF(((IS_VERTICAL) ? 'height' : 'width'), 'main:not([hidden])') + COL_GAP_PX) * increment + ((0<increment) ? COL_RULE_WIDTH_PX : 0);
//    const PAGE_PX = (cssF(((IS_VERTICAL) ? 'height' : 'width'), 'main:not([hidden])') + COL_GAP_PX - COL_RULE_WIDTH_PX) * increment + ((0<increment) ? COL_RULE_WIDTH_PX : 0);
//    const PAGE_PX = (cssF(((IS_VERTICAL) ? 'height' : 'width'), 'main:not([hidden])') + COL_GAP_PX + (((0<increment) ? -1 : 1 ) * COL_RULE_WIDTH_PX)) * increment + ((0<increment) ? COL_RULE_WIDTH_PX : 0);
//    const PAGE_PX = (cssF(((IS_VERTICAL) ? 'height' : 'width'), 'main:not([hidden])') + COL_GAP_PX + (((0<increment) ? -1 : 0 ) * COL_RULE_WIDTH_PX)) * increment + ((0<increment) ? COL_RULE_WIDTH_PX : 0);
    const PAGE_PX = (cssF(((IS_VERTICAL) ? 'height' : 'width'), 'main:not([hidden])') + COL_GAP_PX + (((0<increment) ? -1 : 0 ) * COL_RULE_WIDTH_PX)) * increment + ((0<increment) ? COL_RULE_WIDTH_PX : 0);

//    const PAGE_PX = ((IS_VERTICAL) ? MAIN.scrollHeight : MAIN.scrollWidth) / ALL_PAGE;
//    const PAGE_PX = (((IS_VERTICAL) ? MAIN.scrollHeight : MAIN.scrollWidth) + (COL_GAP_PX/2)) / ALL_PAGE;

//    const PAGE_PX = (((IS_VERTICAL) ? MAIN.clientHeight : MAIN.clientWidth) + COL_GAP_PX + (COL_RULE_WIDTH_PX * (0<increment) ? 1 : -1) + ((COL_RULE_WIDTH_PX/2)*((0<increment) ? -1 : 1 ))) * increment * COLUMN_COUNT;

//    const PAGE_PX = cssF('column-width', 'main:not([hidden])');
    console.log(window.screen.height, document.body.clientHeight, MAIN.clientHeight, MAIN.clientWidth, cssF('width', 'main:not([hidden])'));
    const X_INC = (IS_VERTICAL) ? 0 : PAGE_PX;
    const Y_INC = (IS_VERTICAL) ? PAGE_PX : 0;
//    window.scrollBy(X_INC, Y_INC);
    MAIN.scrollBy(X_INC, Y_INC);
}
...
//        const PAGE_PX = (IS_VERTICAL) ? MAIN.clientHeight : MAIN.clientWidth; // １行の表示領域
        ...
        if (0 <= page) {
            ...
            return (PAGE_PX + COL_GAP_PX + (COL_RULE_WIDTH_PX * (0<increment) ? 1 : -1) + ((0<increment) ? 0 : 1)) * page * COLUMN_COUNT;
        }
        else {
            const TARGET_PAGE = ALL_PAGE + page;
            const increment = TARGET_PAGE - NOW_PAGE;
            return (PAGE_PX + COL_GAP_PX + (COL_RULE_WIDTH_PX * (0<increment) ? 1 : -1) + ((0<increment) ? 0 : 1)) * (TARGET_PAGE * COLUMN_COUNT);
        }
    }
